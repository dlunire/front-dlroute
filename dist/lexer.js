...
function scanner(uri = '') {
    ...
    while (offset < size) {
        ...
        const pick = input[offset + 1] ?? null;
...
function nextDelimiter() {
     ...
     */
    const lexeme = input.substring(currentOffset, end ?? offset);
    tokens.push({
 ...
 */
export function getTokensFromURI(uri) {
    scanner(uri);
    return [...tokens];
}
/**
 ...
 */
export function getTokensFromURL(url) {
    processURL(url);
    return [...tokens];
}
/**
 ...
 * // → "/users/123"
 */
export function getURIFromURL(url) {
    processURL(url);
 ...
 */
export function getURIFromURI(uri) {
    scanner(uri);
    return getCanonicalURI();
}
/**
...
function getTokenType(lexeme) {
    const tokenType = lexeme[0] === COLON
        ? TokenType.Parameter
        : TokenType.Static;
 ...
 * // }
 */
export function parseRoute(uri) {
    resetState();
    uri = getURIFromURI(uri);
    let tokenType = TokenType.Static;
