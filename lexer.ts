 ...
 * la URL.
 */
export function getURLFromURL(stringURL: string | null = null): URL {
    /** URL a normalizar */
    const resolvedURL: string | null = stringURL ?? globalThis.location?.href ?? null;

    if (resolvedURL === null) {
        throw new Error(
            "No se proporcionó una URL y no fue posible obtener la URL actual."
        );
    }

    const url: URL = new URL(resolvedURL);
